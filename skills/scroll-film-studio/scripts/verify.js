...
async function ready(page) {
  await page
    .waitForFunction("window.__ready === true", { timeout: 45000 })
    .catch(() => {
      throw new Error(
        "window.__ready never fired — page not ready, refusing to capture (implement the dev contract)",
      );
    });
}
...
async function jank(url) {
  await withBrowser(async (b) => {
      ...
      () =>
        new Promise((res) => {
          ...
          const tick = () => {
            ...
            last = now;
            y += 13;
            window.scrollTo(0, Math.min(y, end));
    ...
    console.log(
      stats.max < 50
        ? "PASS (max < 50ms)"
        : "JANK — investigate the bitmap window / DPR / frame weight",
    );
...
(async () => {
  if (mode === "shot") await shot(url, out, w, h);
    ...
    console.error(
      "usage: node verify.js shot <url> <out.png> [w] [h]  |  node verify.js jank <url>",
    );
    process.exit(1);
  }
})().catch((e) => {
