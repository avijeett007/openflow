...
- SSIM under-reads on stochastic texture (clouds ~0.66, embers ~0.72, liquid caustics
  ~0.60 can all be seamless). The number says _where_ to look; the side-by-side decides.
- The #1 real failure is **grade/geometry drift** (an invented sunrise, a new horizon).
  Fix by regenerating with: _"Continue the exact same shot from the reference frame,
  identical framing, identical colour grade. Do not change the colour grade."_
...
- `--generate-audio false` is _the_ cost lever — audio ON silently ~3×'s the bill.
...
- **ImageBitmap sliding window**: `drawImage(HTMLImageElement)` forces a _synchronous_ JPEG
  decode on first paint (and after cache eviction) — that decode spike _is_ the frame-by-
...
- **Seamless handoff**: start the next section's background gradient at the _sampled_ final-
...
- Capture: `goto → waitForFunction(__ready) → wait ~1200ms → screenshot`. Shoot every beat
  position _and_ every junction. Hide any cursor-follower until first real mousemove or it
